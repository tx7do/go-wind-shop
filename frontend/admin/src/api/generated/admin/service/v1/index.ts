...
export interface CommentService {
  ...
  Count(
    request: pagination_PagingRequest,
  ): Promise<commentservicev1_CountCommentResponse>;
  Get(
...
export function createCommentServiceClient(
  ...
  return {
    List(request) {
      const path = `admin/v1/mall/comments`;
      const body = null;
      const queryParams: string[] = [];
      if (request.page) {
        queryParams.push(
    ...
    Count(request) {
      const path = `admin/v1/mall/comments/count`;
      const body = null;
      const queryParams: string[] = [];
      if (request.page) {
        queryParams.push(
      ...
      return transport.unary(uri, 'GET', body, {
        service: 'CommentService',
        method: 'Count',
      }) as Promise<commentservicev1_CountCommentResponse>;
    },
    Get(request) {
      if (request.id === undefined || request.id === null) {
        throw new Error('missing required field request.id');
      }
      const path = `admin/v1/mall/comments/${request.id}`;
      const body = null;
      const queryParams: string[] = [];
      if (request.viewMask) {
        queryParams.push(
...
export type commentservicev1_Comment_Status =
  ...
  | 'STATUS_SPAM'
  | 'STATUS_UNSPECIFIED';
export type commentservicev1_CountCommentResponse = {
  count: number | undefined;
};
